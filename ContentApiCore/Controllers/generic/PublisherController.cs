    ...
    public class PublisherController : OdhController
    {
        ...
        public Task<IActionResult> Post([FromBody] PublisherLinked publisher)
        {
            return DoAsyncReturn(async () =>
            {
                ...
                publisher.Id = publisher.Key.ToLower().Replace(" ", "") ?? IdGenerator.GenerateIDFromType(publisher);

                if (publisher.LicenseInfo == null)
                    publisher.LicenseInfo = new LicenseInfo() { ClosedData = false };

                return await UpsertData(publisher, new DataInfo("publishers", CRUDOperation.Create), new CompareConfig(false, false), new CRUDConstraints(additionalfilter, UserRolesToFilter));
            });
        }
        ...
        public Task<IActionResult> Put(string id, [FromBody] PublisherLinked publisher)
        {
            return DoAsyncReturn(async () =>
            {
                ...
                publisher.Id = IdGenerator.CheckIdFromType<PublisherLinked>(id);

                return await UpsertData(publisher, new DataInfo("publishers", CRUDOperation.Update), new CompareConfig(false, false), new CRUDConstraints(additionalfilter, UserRolesToFilter));
            });
        }
        ...
        public Task<IActionResult> Delete(string id)
        {
            return DoAsyncReturn(async () =>
            {
                ...
                id = IdGenerator.CheckIdFromType<PublisherLinked>(id);
