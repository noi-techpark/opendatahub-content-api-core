...
namespace ContentApiCore.Controllers.generic
{
    ...
    public class SourceController : OdhController
    {
            ...
            CancellationToken cancellationToken)
        {
            return DoAsyncReturn(async () =>
            {
                ...
                var idlist = CommonListCreator.CreateIdList(idfilter);
                var typeslist = CommonListCreator.CreateIdList(typefilter);
        ...
        public Task<IActionResult> Post([FromBody] SourceLinked source)
        {
            return DoAsyncReturn(async () =>
            {
                ...
                source.Id = source.Key.ToLower().Replace(" ", "") ?? IdGenerator.GenerateIDFromType(source);

                if (source.LicenseInfo == null)
                    source.LicenseInfo = new LicenseInfo() { ClosedData = false };

                return await UpsertData(source, new DataInfo("sources", CRUDOperation.Create), new CompareConfig(false, false), new CRUDConstraints(additionalfilter, UserRolesToFilter));
            });
        }
        ...
        public Task<IActionResult> Put(string id, [FromBody] SourceLinked source)
        {
            return DoAsyncReturn(async () =>
            {
                ...
                source.Id = IdGenerator.CheckIdFromType<SourceLinked>(id);

                return await UpsertData(source, new DataInfo("sources", CRUDOperation.Update), new CompareConfig(false, false), new CRUDConstraints(additionalfilter, UserRolesToFilter));
            });
        }
        ...
        public Task<IActionResult> Delete(string id)
        {
            return DoAsyncReturn(async () =>
            {
                ...
                id = IdGenerator.CheckIdFromType<SourceLinked>(id);
